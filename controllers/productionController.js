...
const updateProductionPlan = async (req, res, next) => {
  try {
    ...
    // If updating details, validate products
    if (req.body.details) {
...
const deleteProductionPlan = async (req, res, next) => {
  try {
    ...
    // Only allow deletion if status is Planned or Cancelled
...
/**
 * @desc    Complete a production item and auto-create batch
 * @route   POST /api/production/:planId/complete-item
 * @access  Private (Admin, Manager, KitchenStaff)
 * @crucial This is the core logic that auto-creates batches
 */
const completeProductionItem = async (req, res, next) => {
  try {
    ...
    // Get product details for shelf life and SKU
...
/**
 * @desc    Update production plan status
